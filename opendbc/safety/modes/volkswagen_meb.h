...
static safety_config volkswagen_meb_init(uint16_t param) {
  ...
  static const CanMsg VOLKSWAGEN_MEB_STOCK_TX_MSGS[] = {{MSG_HCA_03, 0, 24, .check_relay = true}, {MSG_EA_01, 0, 8, .check_relay = false}, {MSG_EA_02, 0, 8, .check_relay = false},
                                                        {MSG_GRA_ACC_01, 0, 8, .check_relay = false}, {MSG_GRA_ACC_01, 2, 8, .check_relay = false}, {MSG_LDW_02, 0, 8, .check_relay = false}};
...
static void volkswagen_meb_rx_hook(const CANPacket_t *msg) {
  if (msg->bus == 0U) {
    ...
    if (msg->addr == MSG_Motor_54) {
      int accel_pedal_value = msg->data[21] - 37U;
...
static bool volkswagen_meb_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == MSG_HCA_03) {
    ...
    bool steer_req = ((msg->data[1] >> 4) & 0x7U) == 4U;
    int steer_power = msg->data[2] * 0.4;
